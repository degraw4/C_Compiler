...
void External_dec() {   //退出外部声明时 pcur为当前声明的:或函数的} 
	if (pcur == word + wordlen) {
		longjmp(jmpbuf, SUCC);
		return;
	}
	else if (pcur->token == TK_REF) {
	}
...
//外部声明调用变量/函数  变量 pcur为分号退出   函数 pcur为分号or}退出
void type_Statement() {
	if ((pcur+1)->token == TK_IDENTV) {
		int i = pcur->token;
		NextNode();
		NextNode();
		Variate(i);
	}
	else if ((pcur + 1)->token == TK_IDENTF) {
		Func();
	}
	else {
		Row = pcur->row;
